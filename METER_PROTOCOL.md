...
- **The integration should be a dedicated `dlms-gateway` microservice** sitting at the edge (or regional concentrator), upstream of the existing oracle-bridge and chain-bridge. It terminates DLMS associations, normalizes COSEM objects to a canonical schema (preserving OBIS code, scaler/unit, timestamp, status, system-title and invocation counter), batches 15-minute intervals into a Merkle commitment, re-signs with an edge secure element (ATECC608/SE050), and crosschecks against Switchboard TEE attestation before the chain-bridge's policy engine authorizes a Solana transaction.
...
#### B9. Edge gateway / oracle-bridge architecture for DLMS ingestion
...
- **Edge gateway pattern**: meter → DLMS client (association + GET/selective-access or DataNotification listener) → normalization → message bus (NATS JetStream / MQTT / Kafka) → oracle-bridge → chain-bridge.
...
1. **Component placement**: build a dedicated **`dlms-gateway` microservice** (edge or per-island concentrator) that owns DLMS associations, ciphering, selective-access reads and push listening. The existing **oracle-bridge** consumes normalized+attested records; the **chain-bridge** stays protocol-agnostic and enforces policy + signs Solana tx via Vault Transit. Do **not** put DLMS parsing in the chain-bridge.
