...
class StrategyService:
    ...
    def resolve_transmission_bottleneck(
        ...
        payoff_matrix = {
            "S1_GRID": {
                StrategyState.NORMAL: self.financials.retail_price - self.financials.grid_import_cost,
                StrategyState.PEAK: -self.financials.blackout_penalty
            },
            "S2_BESS": {
                StrategyState.NORMAL: self.financials.retail_price - self.financials.bess_lcos,
                StrategyState.PEAK: self.financials.retail_price - self.financials.bess_lcos
            },
            "S3_DIESEL": {
                StrategyState.NORMAL: self.financials.retail_price - self.financials.diesel_gen_cost,
                StrategyState.PEAK: self.financials.retail_price - self.financials.diesel_gen_cost
            }
        }
        ...
        logger.info(f"STRATEGY DECISION: State={state.value}, Selected={best_strategy}, "
    ...
    def calculate_optimal_dispatch_schedule(
        ...
        for t in range(n_steps):
            ...
            cost_total = (p_grid * self.financials.grid_import_cost) + \
            ...
            savings_vs_legacy = p_bess * (self.financials.diesel_gen_cost - self.financials.bess_lcos)
    ...
    def get_financial_summary(self, dispatch_history: List[Dict[str, float]]) -> Dict[str, float]:
        """
        ...
        for event in dispatch_history:
            ...
            total_savings += p_bess * duration * (self.financials.diesel_gen_cost - self.financials.bess_lcos)
