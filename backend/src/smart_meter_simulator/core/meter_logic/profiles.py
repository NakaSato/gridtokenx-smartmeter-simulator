...
def calculate_solar_generation(timestamp: datetime, config: dict, current_weather: str, last_noise: float) -> tuple:
    ...
    weather_factors = {
        "Sunny": 1.0, "Partly Cloudy": 0.7, "Cloudy": 0.4, 
        "Rainy": 0.1, "Stormy": 0.05, "Eclipse": 0.01
    }
...
def calculate_consumption(timestamp: datetime, config: dict, meter_id: str, last_noise: float) -> tuple:
    ...
    if meter_type in [MeterType.RESIDENTIAL, MeterType.SOLAR_PROSUMER, MeterType.HYBRID_PROSUMER]:
        m_peak = 0.8 * math.exp(-((hour - (7.5 + meter_offset * 1.5)) ** 2) / (2 * 1.2 ** 2))
        e_peak = 1.5 * math.exp(-((hour - (18.5 + meter_offset * 2.0)) ** 2) / (2 * 2.5 ** 2))
