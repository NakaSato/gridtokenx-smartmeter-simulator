...
class ReadingManager:
    ...
    def generate_all(
        ...
        if mode == SimulationMode.PLAYBACK and playback_profile:
            playback_data = self.data_source.get_values_batch(
                playback_profile, timestamp
            )
            self._apply_playback_overrides(meters, playback_data)

        # 3. Batch Rust generation (Fast path)
        readings, success = self._generate_rust_batch(
            meters, timestamp, interval, weather_mode, grid_stress
        )
        if success:
            return readings, playback_data

        # 4. Python per-meter generation (Slow path)
        readings = self._generate_python_loop(
            meters, timestamp, interval, playback_data, grid_stress
        )
        return readings, playback_data

    def _apply_playback_overrides(
        self, meters: List[Any], playback_data: Dict[str, Any]
    ):
        for m in meters:
    ...
    def _generate_rust_batch(
        ...
        try:
            ...
            for meter, rr in zip(meters, rust_readings):
                reading = self._create_reading_from_rust(
                    meter, rr, timestamp, interval, weather
                )
    ...
    def _generate_python_loop(
        ...
        for meter in meters:
            ...
            cons_override = playback.get(f"{meter.meter_id}_CONS") or playback.get(
                meter.meter_id
            )

            if hasattr(meter, "manual_override_gen"):
                gen_override = meter.manual_override_gen
            if hasattr(meter, "manual_override_cons"):
                cons_override = meter.manual_override_cons
