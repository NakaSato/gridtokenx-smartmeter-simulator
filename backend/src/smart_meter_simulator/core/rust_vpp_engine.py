...
try:
    from gridtokenx_sim import (
        VPPDispatchEngine as RustVPPDispatchEngine,
        DERResource as RustDERResource,
    )
...
class RustAcceleratedVPP:
    ...
    def dispatch(
        ...
        if self.rust_engine:
            ...
            for r in resources:
                soc_percent = (
                    (r.get("current_soc_kwh", 0) / r.get("capacity_kwh", 1)) * 100.0
                    if r.get("capacity_kwh", 0) > 0
                    else 0.0
                )
                ...
                weight = (soc_w * 0.3 + price_w * 0.4 + carbon_w * 0.3) * r.get(
