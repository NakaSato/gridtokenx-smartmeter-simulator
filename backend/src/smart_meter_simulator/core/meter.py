...
class SmartMeter:
    ...
    def generate_reading(
        ...
        if meter_type in (MeterType.EV_CHARGER, MeterType.DC_FAST_CHARGER):
    ...
    def _calculate_consumption(self, timestamp: datetime) -> float:
        ...
        if meter_type in [MeterType.RESIDENTIAL, MeterType.SOLAR_PROSUMER, MeterType.HYBRID_PROSUMER]:
            ...
            # DC Fast Chargers have moderate base consumption (cooling, standby)
    ...
    def _calculate_dc_charger_behavior(self, timestamp: datetime) -> tuple:
        ...
        High power charging, no V2G, multi-port utilization.
        Returns (generation_kwh, consumption_kwh)
        """
        hour = timestamp.hour + timestamp.minute / 60.0
        config = get_config()
        ...
        # Higher utilization during daytime (8 AM - 10 PM)
        if 8 <= hour <= 22:
            # Peak utilization: 60-95% of connectors active
            ...
            # Low utilization (midnight - 6 AM): 10-30%
            utilization = random.uniform(0.1, 0.3)

        # Get connector count and charge rate
        ...
        # Base consumption without load balancing
        base_consumption_kw = charge_rate_kw * active_ports

        # Apply load balancing: cap at max station capacity
        if base_consumption_kw > max_station_capacity_kw:
            # Distribute available capacity across active ports
            actual_consumption_kw = max_station_capacity_kw
        else:
            actual_consumption_kw = base_consumption_kw

        # Apply charging curve (CC-CV: constant current then constant voltage)
        # Higher SoC = lower charging rate (tapering)
        soc_fraction = self.battery_level / 100.0
        if soc_fraction > 0.8:
            # CV phase: charging rate drops
            taper_factor = 1.0 - ((soc_fraction - 0.8) / 0.2) * 0.6  # Drops to 40% at 100%
            actual_consumption_kw *= taper_factor
        elif soc_fraction < 0.2:
            # Low SoC: slightly reduced rate (battery warming up)
            warmup_factor = 0.7 + (soc_fraction / 0.2) * 0.3  # 70% at 0%, 100% at 20%
            actual_consumption_kw *= warmup_factor

        # Convert to kWh (per interval)
        cons_kwh = actual_consumption_kw / 4.0

        # Update SoC (vehicle battery)
        capacity_kwh = self.config.get('ev_battery_capacity', 60.0)
        if capacity_kwh > 0:
            self.battery_level = min(100.0, self.battery_level + (cons_kwh / capacity_kwh) * 100)

        # DC fast chargers don't generate (no V2G)
        return 0.0, cons_kwh
