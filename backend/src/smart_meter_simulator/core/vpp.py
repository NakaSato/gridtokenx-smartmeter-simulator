...
class DERResource:
    ...
    def max_flexibility_up_kw(self) -> float:
        ...
        if not self.enabled or not self.is_controllable:
            return 0.0
...
class VPPCluster:
    ...
    def calculate_health_score(self) -> float:
        ...
        if not self.resources:
            return 0.0
        avg_rep = sum(r.reputation_score for r in self.resources.values()) / len(
            self.resources
        )
        avg_soc = sum(r.soc_percent for r in self.resources.values()) / len(
            self.resources
        )
...
class VPPManager:
    ...
    def update_meter_state(
        ...
        if meter_id not in self.meter_map:
            return
        ...
        if delta > max_delta and len(resource.history) > 0:
            logger.warning(
                f"VPP SECURITY ALERT: Impossible SOC jump for meter {meter_id} ({prev_soc:.2f} -> {battery_level:.2f})"
            )
    ...
    def dispatch_cluster(
        ...
        for r in resources:
            ...
            soc_w = (
                r.soc_percent / 100.0
                if target_kw > 0
                else (100.0 - r.soc_percent) / 100.0
            )
            ...
            carbon_w = (
                (c_intensity / 500.0)
                if target_kw > 0
                else (1.0 - (c_intensity / 500.0))
            )

            # Combined Weight
            weights[r.meter_id] = (
                soc_w * 0.3 + price_w * 0.4 + carbon_w * 0.3
            ) * r.reputation_score

        # Normalize weights and allocate
        total_w = sum(weights.values())
        if total_w <= 0:
            return {r.meter_id: target_kw / len(resources) for r in resources}
        ...
        if target_kw > 0 and carbon_intensity is not None:
            ...
            logger.info(
                f"VPP: Saved {savings:.2f}g CO2 by displacing {target_kw:.2f}kW grid power"
            )
    ...
    def get_cluster_status(self, cluster_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_soc_percent": (
                cluster.current_stored_kwh / cluster.total_capacity_kwh * 100
            )
            if cluster.total_capacity_kwh > 0
            else 0,
    ...
    def orchestrate_microgrid_stability(
        self, cluster_id: str, freq: float, total_cons: float, total_gen: float
    ):
        """
        ...
        if freq < 49.5:
            ...
            resources = sorted(
                cluster.resources.values(), key=lambda x: x.priority, reverse=True
            )
            ...
            for r in resources:
                if r.priority > 1 and not r.is_shed:
                    ...
                    logger.warning(
                        f"VPP EMERGENCY: Shedding load {r.meter_id} (Priority {r.priority}) to stabilize grid."
                    )
    ...
    def resolve_bottleneck_game(
        ...
        best_strategy, reduction_needed_kw = (
            self.strategy_service.resolve_transmission_bottleneck(
        ...
        if best_strategy == "S2_BESS":
            ...
            if samui_cluster:
                ...
                if bess and bess.max_flexibility_up_kw > 0:
                    ...
                    logger.info(
                        f"STRATEGY S2 (BESS): Discharging {dispatch:.2f} kW. "
            ...
            if reduction_needed_kw > 0:
                ...
                if tao_cluster:
                    ...
                    if diesel:
                        ...
                        f = self.strategy_service.financials
                        logger.warning(
                            f"FALLBACK S3 (DIESEL): Ramping {dispatch:.2f} kW. "
            ...
            if tao_cluster:
                ...
                if diesel:
                    ...
                    reduction_needed_kw -= dispatch
                    logger.info(
                        f"STRATEGY S3 (DIESEL): Ramping {dispatch:.2f} kW to avert blackout."
                    )

        if reduction_needed_kw > 0:
            logger.error(
                f"BOTTLENECK UNRESOLVED: Strategy {best_strategy} insufficient. "
