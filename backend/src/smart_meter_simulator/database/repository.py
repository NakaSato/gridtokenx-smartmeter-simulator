...
from .models import Substation, Meter, MeterReading, PowerPlant, Base
...
class PostGISRepository:
    ...
    async def batch_import_plants(
        ...
        async with self.async_session_factory() as session:
            for idx, plant_data in enumerate(plants_data):
                try:
                    plant = PowerPlant(
                        ...
                        carbon_intensity_gco2_kwh=plant_data.get(
                    ...
                    error_msg = (
                        f"Plant {idx} ({plant_data.get('name', 'unknown')}): {str(e)}"
                    )
            ...
            try:
                await session.commit()
                logger.info(
                    f"Batch import complete: {created} plants created, {len(errors)} errors"
                )
            except Exception as e:
        ...
        return {"created": created, "errors": len(errors), "error_details": errors[:10]}
    ...
    async def get_power_plant(self, plant_id: str) -> Optional[Dict[str, Any]]:
        ...
        async with self.async_session_factory() as session:
            ...
            return {
                ...
                "region": plant.region,
                "voltage_level_kv": float(plant.voltage_level_kv)
                if plant.voltage_level_kv
                else None,
                "grid_connection_type": plant.grid_connection_type,
                "is_renewable": plant.is_renewable,
                "carbon_intensity_gco2_kwh": float(plant.carbon_intensity_gco2_kwh)
                if plant.carbon_intensity_gco2_kwh
                else None,
                "source": plant.source,
    ...
    async def get_power_plants(
        ...
        async with self.async_session_factory() as session:
            ...
            if renewable_only:
                query = query.where(
                    PowerPlant.plant_type.in_(
                        ["hydropower", "solar", "wind", "bioenergy", "geothermal"]
                    )
                )
    ...
    async def get_power_plants_near(
        ...
        async with self.async_session_factory() as session:
            ...
            for p in plants:
                dist = (
                    (float(p.latitude) - latitude) ** 2
                    + (float(p.longitude) - longitude) ** 2
                ) ** 0.5 * 111.0
                if dist <= radius_km:
                    out.append(
                        {
    ...
    async def get_power_plant_stats(self) -> Dict[str, Any]:
        ...
        async with self.async_session_factory() as session:
            ...
            total_query = select(func.count(), func.sum(PowerPlant.capacity_mw)).where(
                PowerPlant.status == "operating"
            )
