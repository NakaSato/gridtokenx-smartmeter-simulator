...
from smart_meter_simulator.database.models import Base
...
def ingest_data():
    ...
    with Session(engine) as session:
            ...
            for code, d in subs_json.items():
                ...
                session.execute(
                    stmt,
                    {
                        ...
                        "type": d["type"],
                        "capacity_mva": d.get("capacity_mva"),
        ...
        if lines_path.exists():
            ...
            res = session.execute(
                text("SELECT id, code, name, longitude, latitude FROM grid.substations")
            )
            ...
            for idx, d in enumerate(lines_json):
                ...
                if from_sub and to_sub:
                    path_json = {
                        "type": "LineString",
                        "coordinates": [
                            [float(from_sub.longitude), float(from_sub.latitude)],
                            [float(to_sub.longitude), float(to_sub.latitude)],
                        ],
                    }
                    stmt = text("""
                    ...
                    session.execute(
                        stmt,
                        {
                            "name": f"{from_sub.name} - {to_sub.name}",
                            "code": code,
                            "from_id": from_sub.id,
                            "to_id": to_sub.id,
                            "voltage_kv": d["voltage_kv"],
