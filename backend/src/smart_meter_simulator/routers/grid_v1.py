...
async def grid_topology(version: Optional[str] = Query(None, description="Topology version")):
...
async def grid_state_estimation():
    ...
    return {"converged": False, "results": {}, "message": "State estimation disabled (ML/AI removal)"}
...
async def list_substations():
    ...
    return {
        "substations": [
            {"id": "TH-SUB-001", "name": "Bangkhen Substation", "operator": "MEA", "voltage_kv": 22.0},
            {"id": "TH-SUB-002", "name": "Pathumwan Substation", "operator": "MEA", "voltage_kv": 22.0}
        ],
        "total": 2
    }
