...
async def import_power_plants_geojson(
    ...
    try:
        ...
        for idx, feature in enumerate(geojson.get('features', [])):
            ...
            plant_type_raw = (
                props.get('Type') or
                props.get('plant_type') or
                props.get('source') or
                'unknown'
            ).lower().replace(' ', '_')

            capacity = (
                props.get('Capacity (MW)') or
                props.get('capacity_mw') or
                props.get('output_mw') or
                0
            )
            ...
            name = (
                props.get('Plant / Project name') or
                props.get('name') or
                f"Plant_{idx}"
            )
        ...
        if not plants_data:
            return BatchImportResponse(created=0, errors=0, error_details=["No valid features found"])
        ...
        try:
            ...
            return BatchImportResponse(
                ...
                errors=0,
                error_details=[f"DB unavailable. {len(plants_data)} plants saved to {filepath}"]
            )
...
async def create_power_plant(
    ...
    try:
        ...
        if "unique constraint" in str(e).lower() or "duplicate" in str(e).lower():
            raise HTTPException(status_code=409, detail=f"Plant {plant.plant_id} already exists")
    ...
    if repo is None or not os.path.exists(filepath):
        if os.path.exists(filepath):
            with open(filepath) as f:
                geojson = json.load(f)
            renewable_sources = {'solar', 'wind', 'hydro', 'biofuel', 'biogas', 'biomass', 'geothermal'}
            plants = []
            for idx, feat in enumerate(geojson.get('features', [])):
                ...
                if plant_type and src != plant_type:
                    continue
                cap = p.get('Capacity (MW)') or p.get('capacity_mw') or p.get('output_mw') or 0
                try:
                ...
                plants.append({
                    ...
                    "name": p.get('Plant / Project name') or p.get('name') or f"Plant_{idx}",
                    "plant_type": src,
                    "capacity_mw": cap,
    ...
    if os.path.exists(filepath):
        ...
        for feat in geojson.get('features', []):
            ...
            if src in renewable_sources:
                renewable_mw += cap
        return {
            "total": {"count": len(geojson.get('features', [])), "capacity_mw": total_mw},
            "renewable": {"capacity_mw": renewable_mw, "percentage": (renewable_mw / total_mw * 100) if total_mw > 0 else 0},
            "by_type": {k: {"plant_count": v, "total_capacity_mw": 0, "avg_capacity_mw": 0} for k, v in sources.items()},
            "_source": "fallback_file"
        }
    raise HTTPException(status_code=404, detail="No plant data available. Import a GeoJSON file first.")
