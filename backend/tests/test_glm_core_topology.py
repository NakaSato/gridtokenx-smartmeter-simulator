...
def test_reference_glm_loads_as_core_topology():
    ...
    # 76 lines + 3 PCC transformers: the zone heads hang off transformers (which
    # is what makes each group a zone), not off lines to the MV busbar. The
    # fourth physical feeder (the B78 stub) is served from zone 3 over a line.
    assert len(topology.lines) == 76
    assert len(topology.transformers) == 3
    assert set(topology.zones) == {1, 2, 3}
    assert topology.zones[3].member_buses[-3:] == (
        "ref_lv_bus_78",
        "ref_lv_bus_79",
        "ref_lv_bus_80",
    )
...
def test_glm_topology_adapter_loads_reference_glm():
    ...
    assert adapter.get_line_count() == 76
