...
import { FinancialsTab } from '@/components/dashboard/components/FinancialsTab';
import { LoadShedControl } from '@/components/dashboard/components/LoadShedControl';
...
const Dashboard = () => {
    const {
        ...
        handleControl, updateEnvironment, updateMeterCount, handleAttack, clearLogs, fetchInitialMeters
    } = useSimulator();
    ...
    const [activeTab, setActiveTab] = useState<'meters' | 'financials'>('meters');
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="flex border-b border-slate-800 gap-6">
                        <button
                            ...
                            className={cn(
                                "pb-3 font-bold text-sm tracking-wider uppercase border-b-2 transition-all duration-300 cursor-pointer",
                                activeTab === 'meters' 
                        ...
                        <>
                            ...
                            <div className={cn(viewType === 'grid' ? "grid grid-cols-1 md:grid-cols-2 gap-4" : "flex flex-col gap-2", "min-h-[400px]")}>
                                {paginatedMeters.map(meter => (
                                    viewType === 'grid' 
                                        ? <MeterCard key={meter.meter_id} reading={meter} onEdit={handleEditMeter} onMeta={handleEditMetadata} /> 
                                        : <MeterListItem key={meter.meter_id} reading={meter} onEdit={handleEditMeter} onMeta={handleEditMetadata} />
                                ))}
