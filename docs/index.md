# GridTokenX Smart Meter Simulator Documentation

Welcome to the documentation for the **GridTokenX Smart Meter Simulator**. This system provides high-fidelity AMI simulation and VPP grid orchestration.

---

## 🏁 Quick Start & Installation
- [Getting Started](guides/getting-started.md) - Installation and basic setup.
- [Configuration Guide](guides/configuration.md) - Environment variables and settings.
- [Running Simulations](guides/running-simulations.md) - How to start and manage simulations.

## 🚀 Core Features
- [Rust Acceleration](wiki/integration/rust-acceleration.md) - High-performance telemetry generation.
- [Early Warning System](features/GRID_NOTIFICATIONS.md) - Submarine cable and grid monitoring.
- [Cost Optimization](features/COST_REPORTING.md) - OPF and diesel displacement analysis.

## 📖 Reference & Standards
- [API Design](reference/API_DESIGN.md) - REST API and WebSocket specification.
- [Thai Grid Tariffs](reference/thai-tariffs.md) - TOU and residential rates for Thailand (2026).
- [Meter Specification](reference/meter-spec.md) - AMI accuracy classes and data fields.
- [Thai Grid Topology](reference/thai-grid-topology.md) - MEA/PEA distribution models.
- [Thai Market Analysis](reference/thai-market.md) - Economic modeling of the Thai energy sector.

## 🏗️ Architecture
- [System Overview](architecture/overview.md) - High-level component diagram.
- [Simulation Engine](architecture/simulation-engine.md) - Core orchestration logic.
- [Smart Meter Model](architecture/smart-meter.md) - Digital twin implementation.
- [Grid Integration](architecture/grid-integration.md) - Pandapower and state estimation.
- [Transport Layer](architecture/transport-layer.md) - gRPC, MQTT, and Kafka delivery.

## 🛠️ Operations & Deployment
- [Deployment Guide](operations/DEPLOYMENT_GUIDE.md) - Consolidated production setup guide.
- [Docker Deployment](guides/docker-deployment.md) - Docker-specific instructions.
- [Production Deployment](operations/PRODUCTION_DEPLOYMENT.md) - Advanced production strategies.

## 📜 Archive
- [Project History](archive/) - Historical plans, status reports, and hackathon strategies.

---

*Maintained by the GridTokenX Engineering Team.*
