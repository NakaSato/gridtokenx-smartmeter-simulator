...
class MarketAgent:
    ...
    def __init__(
        ...
        base_sell_price: float = 0.35,  # $/kWh
        base_buy_price: float = 0.40    # $/kWh
    ):
    ...
    def calculate_prices(
        ...
        sell_price = max(0.10, min(0.60, sell_price))  # 10-60 cents
        buy_price = max(0.15, min(0.70, buy_price))    # 15-70 cents
        
        # Ensure sell < buy (arbitrage-free)
        if sell_price >= buy_price:
            spread = (buy_price - sell_price) / 2 + 0.05
