...
class MeterService:
    ...
    def get_meter_status(self, meter_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "latest_reading": self._format_reading(latest_reading)
                if latest_reading
                else None,
                "coordinates": {
    ...
    def set_meter_override(
        self, meter_id: str, override_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for meter in self.engine.meters:
                try:
                    ...
                    logger.warning(
                        f"Error getting status for meter {meter.meter_id}: {e}"
                    )
    ...
    def _validate_meter_config(self, config: Dict[str, Any]) -> None:
        ...
        if config["meter_type"] not in valid_types:
            raise ValidationError(
                f"Invalid meter type. Must be one of: {', '.join(valid_types)}"
            )
        ...
        if "trading_preference" in config:
            ...
            if config["trading_preference"] not in valid_preferences:
                raise ValidationError(
                    f"Invalid trading preference. Must be one of: {', '.join(valid_preferences)}"
                )
